...
path_save_rgb = r'C:\Lap trinh\realsense\project\images\rgb'
path_save_depth = r'C:\Lap trinh\realsense\project\images\depth_raw'
...
if __name__ == "__main__":
    ...
    print("Nhấn 's' để lưu ảnh, 'q' hoặc 'ESC' để thoát.")
    try:
        while True:
            ...
            if key == ord('s'):
                # Tự động lấy ID ảnh mới
                img_id = get_next_id(path_save_rgb)
                name_rgb = os.path.join(path_save_rgb, f"{img_id:04d}.png")
                name_depth = os.path.join(path_save_depth, f"{img_id:04d}.png")

                cv2.imwrite(name_rgb, rgb)
                cv2.imwrite(name_depth, depth_raw)
                print(f"✅ Saved {name_rgb} and {name_depth}")
